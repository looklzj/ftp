...
func (c *ServerConn) ModTime(path string, uptime string) error {
	_, _, err := c.cmd(StatusCommandOK, "MDTM %s %s", uptime, path)
	return err
}
...
// SysTemTyp 返回系统类型
func (c *ServerConn) SysTemTyp() (string, error) {
	_, res, err := c.cmd(StatusCommandOK, "SYST")
	return res, err
}

// ServerTyp 获取服务器类型
func (c *ServerConn) ServerTyp() (string, error) {
	_, res, err := c.cmd(StatusCommandOK, "STAT")
	return res, err
}
